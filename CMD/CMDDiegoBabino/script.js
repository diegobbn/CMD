...
window.onload = function() {
	...
	input.addEventListener('keyup', function(e) {
		if((e.keyCode || e.which) == 13) {// ENTER key pressed and 13 is speed (high number higher typing speed)
			var targetValue = input.value;
			var destination = "#" + targetValue;
			typeWriter(destination,"true",10); // 17 is speed (high number higher typing speed)
			input.value = "";

			if(sectionArray.includes(targetValue) == false) {
				typeWriter("#error","true",20); // 20 is speed (high number higher typing speed)
			}
		}
	});
};
